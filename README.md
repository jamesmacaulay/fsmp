...
`tests/fixtures/dev-cycle.yaml` encodes an implement-and-review workflow. A skill
points its orchestrator agent at the definition:
...
run (see `tests/fixtures/dev-cycle.yaml`). Tested with `cargo test` (unit tests
inline; integration tests run the binary against the fixture).
